...
def train_fedavg(net_id, net, train_dataloader, epochs, lr, optimizer, weight_decay, device):
...
def main(args):
    ...
    for round in range(1, args.comm_round):
        ...
        for i in range(client_num):
            train_fedavg(i, local_models[i], train_loaders[i], args.epochs, args.lr, args.optimizer, args.weight_decay, device)
        ...
        for i in range(client_num):
            _, test_acc = test(local_models[i], test_loaders[i], loss_fn, device)
